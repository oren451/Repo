...
void CheeseProduct::saveBin(ofstream & out) const
{
	MilkProduct::saveBin(out);
	out.write((char*)&mAddition, sizeof(mAddition));
}

void CheeseProduct::loadBin(ifstream & in)
{
	MilkProduct::loadBin(in);
	in.read((char*)&mAddition, sizeof(mAddition));
}